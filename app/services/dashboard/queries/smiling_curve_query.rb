    ...
    class SmilingCurveQuery
      ...
      def channels_data
        channel_identity_acquisitions.map do |channel, identity_acquisitions|
          {
            channel: channel,
            data: lifecycle_months_for_channel(identity_acquisitions)
          }
        end
      end

      def lifecycle_months_for_channel(identity_acquisitions)
        (1..MAX_LIFECYCLE_MONTHS).map do |month|
          avg_revenue = average_revenue_for_month(identity_acquisitions, month)
          { month: month, avg_revenue: avg_revenue }
        end
      end

      def average_revenue_for_month(identity_acquisitions, month)
        ...
        (revenues.sum / revenues.size).round(2)
      end

      def revenue_for_lifecycle_month(identity_id, acquisition_date, month)
        month_start = acquisition_date + (month - 1).months
        month_end = acquisition_date + month.months
      ...
      def channel_identity_acquisitions
        @channel_identity_acquisitions ||= acquisition_credits
          ...
          .transform_values do |credits|
            credits.to_h { |c| [c[:identity_id], c[:acquisition_date]] }
          end
      end
      ...
      def acquisition_credits
        @acquisition_credits ||= AttributionCredit
          ...
          .map do |channel, identity_id, converted_at|
            { channel: channel, identity_id: identity_id, acquisition_date: converted_at }
          end
      end
