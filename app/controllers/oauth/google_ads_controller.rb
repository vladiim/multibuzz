  ...
  class GoogleAdsController < ApplicationController
    ...
    def select_account
      ...
      @already_connected_ids = oauth_account.ad_platform_connections
        .where(platform: :google_ads).pluck(:platform_account_id).to_set
    end
    ...
    def create_connection
      ...
      redirect_to outcome[:clear_session] ? google_ads_account_integrations_path : oauth_google_ads_select_account_path,
        **outcome.except(:clear_session)
    end

    def done
      clear_oauth_session!
      redirect_to google_ads_account_integrations_path
    end
