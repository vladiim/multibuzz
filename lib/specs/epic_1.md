...
1. **Setup and Infrastructure (Foundation)**
   ...
   - Gem skeleton: `bundle gem multibuzz`. Make it a client library (not engine initially); include HTTP client (e.g., Faraday) for API calls to SaaS.
   - Client app setup: `rails new multibuzz-client --database=postgresql` (minimal, for gem testing).
...
2. **Visitor and Session Management (Identification in Client, Storage in SaaS)**
   ...
   - In gem: Middleware to generate/set visitor IDs (e.g., hash-based); config for API key/endpoint (initializer: `Multibuzz.configure { |config| config.api_url = 'https://multibuzz.com/api', config.api_key = 'xyz' }`). Send IDs to SaaS via POST.
...
3. **Event Tracking (Capture in Client, Ingestion in SaaS)**
   ...
   - In gem: Extend middleware for auto-logging (e.g., page views); helper for customs (`multibuzz_track(:signup, properties: {})`). Queue and send batches to SaaS API.
...
4. **Attribution Computation (Core in SaaS)**
   ...
   - In gem: No compute—query SaaS API for results (e.g., `Multibuzz::Attribution.fetch(model: :linear, visitor_id: id)`).
