# Visit Count 5x Inflation — Investigation, Fix & SDK Navigation Detection

**Date:** 2026-01-29 | **Status:** Investigating | **Severity:** Critical (client-facing data integrity)
...
1. **Navigation detection in middleware**
   - [ ] Add `should_create_session?` method with Sec-Fetch whitelist + blacklist fallback
   - [ ] Gate `create_session_async` call behind `should_create_session?`
   - [ ] Keep visitor cookie setting ungated (all responses)
   - [ ] Remove session cookie completely (`_mbuzz_sid`): delete `set_session_cookie`, `session_cookie_options`, `SESSION_COOKIE_NAME`, `SESSION_COOKIE_MAX_AGE`, `session_id_from_cookie`, `generate_session_id`

2. **Tests**
   - [ ] Real page navigation (Sec-Fetch-Mode: navigate, Sec-Fetch-Dest: document) → creates session
   - [ ] Turbo frame request (Turbo-Frame header) → skips session creation
   - [ ] htmx request (HX-Request header) → skips session creation
   - [ ] fetch()/XHR (Sec-Fetch-Dest: empty) → skips session creation
   - [ ] Prefetch (Sec-Purpose: prefetch) → skips session creation
   - [ ] Old browser (no Sec-Fetch headers, no framework headers) → creates session (fallback)
   - [ ] Old browser with Turbo-Frame header → skips (blacklist fallback)
   - [ ] Visitor cookie set on ALL responses regardless of navigation detection

3. **Release**
   - [ ] Bump to v0.8.0 (breaking: session cookie removed, navigation detection added)
   - [ ] Update CHANGELOG
   - [ ] Update README with navigation detection section
