...
**How to apply:**
- Fix BUILT on branch `fix/reattribution-reliability` (all five phases of `lib/specs/reattribution_reliability_spec.md`): batched session loading, cached and capped conversion paths, coordinator plus bounded chunk jobs, dedicated `reattribution` queue, per-batch concurrency cap, `ReattributionBatch` progress record, chunk wall-clock budget, un-starvable claimed-duration watchdog. Full suite green. NOT yet deployed: production still runs the slow reattribution code until the branch ships.
