...
Rails.application.configure do
  ...
  # Store uploaded files on DigitalOcean Spaces (S3-compatible). Web and jobs
  # run on separate hosts, so local disk is not a shared substrate.
  ...
  # Log to STDOUT (kamal logs) + daily rotated file (archived to DO Spaces).
  config.log_tags = [ :request_id ]
  stdout_logger = ActiveSupport::TaggedLogging.logger(STDOUT)
  file_logger = ActiveSupport::TaggedLogging.new(
    Logger.new(Rails.root.join("log", "production.log.#{Date.current.iso8601}"), 0)
  )
  ...
  config.action_mailer.default_url_options = { host: "mbuzz.co" }
  ...
  config.solid_errors.email_from = "hello@mbuzz.co"
  config.solid_errors.email_to = "vlad@mbuzz.co"
